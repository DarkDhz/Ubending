...
class SearchEngine(Resource):
    ...
    def post(self):
        ...
        if data['name'] is None:
            result = searchByCategory(category_id=data['category'], start_point=start_point, jump=jump)
            if result == 404:
                return {"message": "No products found"}, 400
            return result, 200

        if data['category'] is None:
            result = searchByName(name=data['name'], start_point=start_point, jump=jump)
            if result == 404:
                return {"message": "No products found"}, 400
            return result, 200

        result = searchByCategoryAndName(category_id=data['category'], name=data['name'], start_point=start_point, jump=jump)
        if result == 404:
            return {"message": "No products found"}, 400
        return result, 200
